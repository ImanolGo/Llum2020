...
LedGroup::LedGroup(string& name, unsigned char channel): m_name(name), m_sizeScale(1.0), m_offset(100), m_channel(channel)
{
...
void LedGroup::setPixelColor(ofPixelsRef pixels, int index)
{
    ...
    if(!m_boundingBox2D.inside(m_points2D[index])){
        m_colors[index] = ofColor::black;
        return;
    }
    
    
    int x = (int) ofMap(m_points2D[index].x, m_boundingBox2D.getMinX(), m_boundingBox2D.getMaxX(), 0, pixels.getWidth()-1, true);
    int y = (int) ofMap(m_points2D[index].y, m_boundingBox2D.getMinY(), m_boundingBox2D.getMaxY(), pixels.getHeight()-1,0, true);
...
void LedGroup::removeCharsFromString( string &str, char* charsToRemove ) {
...
void LedGroup::centre2DLeds(float margin_percentage)
{
    ...
    m_boundingBox2D.set(minPos, maxPos);
    
    ofLogNotice() <<"LedGroup::centreLeds -> min position: x = "  << m_boundingBox2D.getMinX() << ", y = "  << m_boundingBox2D.getMinY() ;
    ofLogNotice() <<"LedGroup::centreLeds -> max position: x = "  << m_boundingBox2D.getMaxX() << ", y = "  <<m_boundingBox2D.getMaxY();
