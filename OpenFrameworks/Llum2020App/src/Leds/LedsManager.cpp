...
void LedsManager::normalizeLeds()
{
    ...
    auto box = m_ledGroups.begin()->second->getBoundingBox2D();
    
    glm::vec2 min = box.getMin();
    glm::vec2 max = box.getMax();
    
    for(auto & ledGroup: m_ledGroups){
        box = ledGroup.second->getBoundingBox2D();
    ...
    m_boundingBox2D.set(min, max);
    
    
    //Add margin
    float scalingFactor = 1.2;
    m_boundingBox2D.scaleFromCenter(scalingFactor);
    
    for(auto & ledGroup: m_ledGroups){
        ledGroup.second->setBoundingBox2D(m_boundingBox2D);
    }
...
void LedsManager::createLayout()
{
    ...
    for(auto & ledGroup: m_ledGroups){
        ...
        for(auto led: positions){
            float x = ofMap(led.x, m_boundingBox2D.getMinX(), m_boundingBox2D.getMaxX(), 0.0, width-1);
            float y = ofMap(led.y, m_boundingBox2D.getMinY(), m_boundingBox2D.getMaxY(), height-1,0);
